...
def predict(img_path):
    with st.spinner('Processing..'):
        ...
        for i,(x,y,w,h) in enumerate(table_boundRect):
            ...
            st.image(image_crop, caption=f"Cropped Table {i+1}", use_column_width=True)
            
            # Perform OCR using Tesseract
            ocr_text = perform_ocr(image_crop)

            try:
                df = pd.read_csv(StringIO(ocr_text), sep=r'\|', lineterminator=r'\n', engine='python')
                st.write(f" ## Table {i+1}")
                st.write(df)
            except pd.errors.ParserError:
                try:
                    df = pd.read_csv(StringIO(ocr_text), delim_whitespace=True, lineterminator=r'\n', engine='python')
                    st.write(f" ## Table {i+1}")
                    st.write(df)
                except pd.errors.ParserError:
                    st.write(f" ## Table {i+1}")
